...
class Test_CMB2_Utils extends Test_CMB2
{
    ...
    public function _make_attachment($upload, $parent_post_id = -1)
    {
        $type = '';
    ...
    public function test_get_url_from_dir()
    {
        ...
        foreach ([
            ...
            add_filter('theme_root', ['Test_CMB2_Utils_WIN', '_change_to_wamp_theme_root']);

            $this->assertEquals(
                site_url($located),
                Test_CMB2_Utils_WIN::get_url_from_dir(ABSPATH.$located)
            );
            remove_filter('theme_root', ['Test_CMB2_Utils_WIN', '_change_to_wamp_theme_root']);
        }
    }
    ...
    {
        $this->assertEquals(['test'], CMB2_Utils::ensure_array('', ['test']));
        $this->assertEquals(['test'], CMB2_Utils::ensure_array(false, ['test']));
        $this->assertEquals(['test'], CMB2_Utils::ensure_array(0, ['test']));
    ...
    public function test_php_to_js_dateformat()
    {
        $tests = [
            ...
            ['l', 'DD'],
            ['F', 'MM'],
            ['l jS \of F Y h:i:s A', 'DD dS &#39;o&#39;f MM yy hh:mm:ss TT'],
            ['m.d.y', 'mm.dd.y'],
            ['j, n, Y', 'd, m, yy'],
            ['Ymd', 'yymmdd'],
            // @todo Fix these:
            ['j-m-y, \it \is w Day', 'd-mm-y, &#39;i&#39;t &#39;i&#39;ss w Dtty'],
            ['\i\t \i\s \t\h\e jS \d\a\y.', '&#39;it&#39; &#39;is&#39; &#39;the&#39; dS &#39;day&#39;.'],
        ];
        foreach ($tests as $index => $test) {
            $this->assertSame(
                $test[1],
