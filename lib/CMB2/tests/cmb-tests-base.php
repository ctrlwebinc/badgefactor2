...
abstract class Test_CMB2 extends WP_UnitTestCase
{
    ...
    {
        return [$this->oembed_no_connection_result($url)];
    }
    ...
    protected function assertVerifiersMatch($result_verifiers, $actual)
    {
        ...
        $possibilities = [
            'connected' => array_map([$this, 'normalize_http_string'], $result_verifiers['connected']),
        ];

        if (!empty($result_verifiers['no_connection'])) {
            $possibilities['no_connection'] = array_map([$this, 'normalize_http_string'], $result_verifiers['no_connection']);
        }
    ...
    {
        return $this->capture_render([$field, 'render_field']);
    }

    protected function hook_to_wp_die($hook)
    {
        $this->hooks_to_die[] = $hook;
        add_action($hook, [$this, 'wp_die']);
    }

    public function wp_die()
    {
        $hook = array_pop($this->hooks_to_die);
        remove_action($hook, [$this, 'wp_die']);
        wp_die($hook.' die');
    }
