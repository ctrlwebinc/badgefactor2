...
class Test_CMB2_Field extends Test_CMB2
{
    ...
    public function setUp()
    {
        ...
        $this->field_args = [
            ...
            'before_field'  => [__CLASS__, 'before_field_cb'],
            'after_field'   => 'after_field_static',
            'classes_cb'    => [__CLASS__, 'row_classes_array_cb'],
            'default_cb'    => [__CLASS__, 'cb_to_set_default'],
            'render_row_cb' => [__CLASS__, 'render_row_cb_test'],
            'label_cb'      => [__CLASS__, 'label_cb_test'],
        ];
    ...
    public function test_cmb2_classes_field_callback_with_string()
    {
        ...
        $args['row_classes'] = [__CLASS__, 'row_classes_string_cb'];
    ...
    public function test_filtering_field_type_sanitization_filtering()
    {
        ...
        add_filter('cmb2_sanitize_text', [__CLASS__, '_return_different_value']);
        $modified = $field->save_field('some value to be modified');
        $this->assertTrue((bool) $modified);
        remove_filter('cmb2_sanitize_text', [__CLASS__, '_return_different_value']);
    ...
    public function test_multiple_meta_rows()
    {
        ...
        $array_val = ['check1', 'check2'];

        $cmb_demo = cmb2_get_metabox([
            'id'            => $prefix.'metabox',
    ...
    public function test_set_get_filters()
    {
        ...
        $this->assertFalse((bool) get_post_meta($this->field->object_id, $this->field->_id(), 1));

        // Now filter the setting of the meta.. will use update_option instead
        add_filter("cmb2_override_{$this->field->_id()}_meta_save", [__CLASS__, 'override_set'], 10, 4);
        // Set the value
        $this->field->save_field($array_val);
        ...
        add_filter("cmb2_override_{$this->field->_id()}_meta_value", [__CLASS__, 'override_get'], 10, 4);
        // Get the value
        $value = $this->field->get_data();
    ...
    {
        $field = $this->field->get_field_clone([
            'id' => 'test_field_clone',
        ]);
