...
class Test_CMB2_Types_Display extends Test_CMB2_Types_Base
{
    ...
    public function test_text_datetime_timestamp_timezone()
    {
        ...
        $sanitizer = new CMB2_Sanitize($this->get_field_object('text_datetime_timestamp_timezone'), [
    ...
    {
        $this->assertOptionDisplayFieldMatches('multicheck', ['one', 'two'], 'One, Two');
    }

    public function test_multicheck_inline()
    {
        $this->assertOptionDisplayFieldMatches('multicheck_inline', ['one', 'two'], true, '<div class="cmb-column cmb-type-multicheck-inline cmb2-id-options-test-field cmb-inline" data-fieldtype="multicheck_inline">One, Two</div>');
    }
    ...
    public function test_oembed()
    {
        ...
        $cb = [$this->get_field_object('oembed'), 'render_column'];
    ...
    protected function assertOptionDisplayFieldMatches($type, $value, $value_display = false, $string = false)
    {
        ...
        $this->assertHTMLstringsAreEqual(
            $string,
            $this->capture_render([$field, 'render_column'])
        );
    }
    ...
    protected function assertDisplayFieldMatches($type, $value, $value_display = false, $string = false)
    {
        ...
        $this->assertHTMLstringsAreEqual(
            $string,
            $this->capture_render([$this->get_field_object($type), 'render_column'])
        );
    }
