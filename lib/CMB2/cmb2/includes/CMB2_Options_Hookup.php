...
class CMB2_Options_Hookup extends CMB2_hookup
{
    ...
    public function maybe_register_message()
    {
        ...
        if ($cb = $this->cmb->maybe_callback('message_cb')) {
             ...
             *
             *     @var bool   $is_options_page Whether current page is this options page.
             *     @var bool   $should_notify   Whether options were saved and we should be notified.
             *     @var bool   $is_updated      Whether options were updated with save (or stayed the same).
             *     @var string $setting         For add_settings_error(), Slug title of the setting to which
             *                                   this error applies.
             *     @var string $code            For add_settings_error(), Slug-name to identify the error.
             *                                   Used as part of 'id' attribute in HTML output.
             *     @var string $message         For add_settings_error(), The formatted message text to display
             *                                   to the user (will be shown inside styled `<div>` and `<p>` tags).
             *                                   Will be 'Settings updated.' if $is_updated is true, else 'Nothing to update.'
             *     @var string $type            For add_settings_error(), Message type, controls HTML class.
			...
			<?php if (!empty($tabs)) { ?>
				<h2 class="nav-tab-wrapper">
					<?php foreach ($tabs as $option_key => $tab_title) { ?>
						<a class="nav-tab<?php if (self::is_page($option_key)) { ?> nav-tab-active<?php } ?>" href="<?php menu_page_url($option_key); ?>"><?php echo wp_kses_post($tab_title); ?></a>
					<?php } ?>
				</h2>
			<?php } ?>
